...
class AgentState(TypedDict):
    ...
    salesforce_data: Optional[dict]      # mapped SF account fields
    salesforce_error: Optional[str]      # set if Salesforce call failed
...
def _get_sf_token() -> dict:
    """
    Obtain a Salesforce access token via Client Credentials Flow.
    Returns dict with 'access_token' and 'instance_url'.
    Raises Exception with the error payload if auth fails.
    """
    payload = {
    ...
    if "access_token" not in token_data:
        raise Exception(f"Salesforce auth failed: {token_data}")
    return token_data


# ── Salesforce field mapping ──────────────────────────────────────────────────
# Maps raw Salesforce field values to business-readable labels.
...
def fetch_salesforce_client(state: AgentState) -> AgentState:
    ...
    Auth: Client Credentials Flow via SF_CONSUMER_KEY / SF_CONSUMER_SECRET.
    Query: SELECT Name, Type, CustomerPriority__c, Active__c, Owner.Name
    On failure: sets salesforce_data to None and records the error message.
    """
    ...
    try:
        ...
        # ── SOQL query ────────────────────────────────────────────────────────
        # Fields confirmed working in SF_SQL_testing.py
        soql = (
        ...
        # ── Map to business-readable values ───────────────────────────────────
        salesforce_data = {
            "Name":              raw.get("Name"),
            "Type":              raw.get("Type"),
...
def format_answer(state: AgentState) -> AgentState:
    ...
    # ── Build Salesforce profile block ───────────────────────────────────────
    ...
    if sf:
        ...
        sf_error = state.get("salesforce_error", "unknown error")
        sf_block = f"⚠️ Client profile unavailable — showing operational data only\n_(Salesforce error: {sf_error})_"
